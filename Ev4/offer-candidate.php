...
if(isset($_GET['candidateid']) || isset($_POST['allcheck'])){
	...
	//mysqli_query($link,"Update submitted_candidates set status='Rejected' WHERE id='$submittedid' and employerid='$mid'"); 
	mysqli_query($link,"Update jobs INNER JOIN submitted_candidates ON submitted_candidates.jobid = jobs.jobid set submitted_candidates.status='Offered',submitted_candidates.feedbackdate=now() WHERE submitted_candidates.candidateid='$candidateid' and jobs.memberid='$mid'");

		
			$q2=mysqli_query($link,"select e.email,e.offernotification,d.name,e.firstname from companyprofile d,members e ,submitted_candidates c where  d.id=e.companyid  and e.memberid=c.recruiterid and c.candidateid='$candidateid'");
		...
		while($row3= mysqli_fetch_array($q3)){
			$array3=$row3;
				...
				$content='<p><strong>'.$array3['fname'].'</strong> has been offered. </p>';
		$offernotification=$array2['offernotification'];	
		if($offernotification==1){
	//mail to agency	
		$to = $array2['email'];

			$from = "noreply@recruitinghub.com";
            $r=	getAccountManagerDetail($link,$mid);
            $c=	getCandidatesDetail($link,$candidateid);

			$subject = "RecruitingHub.com - Congrats! ".$array3['fname']." - $candidateid has been offered for ".$array1['jobtitle']." by ".$array1['name']."";
                                      ...
                                      <tr>
                                        <td><p>
                                         Dear '.$array2['firstname'].',<br>
                                          ...
                                          <br>
                                          Congrats! Your profile <strong>'.$array3['fname'].'</strong> - <strong>'.$candidateid.'</strong> submitted for <strong>'.$array1['jobtitle'].'</strong> role from <strong>'.$array1['name'].'</strong> has been <strong>Offered</strong>. <br>
                                          <br>
                                          Please follow up with the candidate until joining and use our "Add Notes" or MESSAGING option to keep the client updated with progress until the onboarding is complete.</p>  Login here <a href="https://www.recruitinghub.com/recruiter/login" target="_blank">Agency Login</a></td>
                                      </tr>
                                      <tr>
                                        <td><p>&nbsp;</p></td>
                                      </tr>
  ...
  //mail to candidate
	...
	<table style="border-right:1px solid #d5d8d9;border-left:1px solid #d5d8d9;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#000" align="center" bgcolor="#792785" border="0" cellpadding="0" cellspacing="0" width="620">
	  ...
	  <tr>
                            ...
                            <td><table border="0" cellspacing="0" cellpadding="0" width="100%">
                                <tr>
                                  <td valign="top"><table border="0" cellspacing="0" cellpadding="0" width="100%">
                                      <tr>
                                        <td><p>
                                         Dear '.$array3['firstname'].',<br>
                                          <br>
                                          Greetings from <a href="https://www.recruitinghub.com" target="_blank">www.recruitinghub.com </a><br>
                                          <br>
                                          Congrats! Your profile <strong>'.$array3['fname'].'</strong> - <strong>'.$candidateid.'</strong> submitted for <strong>'.$array1['jobtitle'].'</strong> role to <strong>'.$array1['name'].'</strong> has been <strong>Offered</strong>. <br>
                                          <br>
		...
		$params = ['from'=>'noreply@recruitinghub.com','name'=>'Recruiting Hub','to'=>$to,
'subject'=>$subject,'message'=>$message,'cc'=>$array2['email'],'send'=>true];
  AsyncMail($params);
  
  echo "<script>
alert('Candidate has been Offered. An email notification has now been sent to the Recruiter');
</script>";
...
<li><a href="add-user" role="tab" > <i class="fa fa-plus"></i>Add Users</a> </li>
 <li><a href="view-all-resource-engagement" role="tab" > <i class="fa fa-database"></i>Request for Your Bench Resources <span class="label label-primary">Free</span></a> </li>  
    <li><a href="resources" role="tab" > <i class="fa fa-database"></i>Bench Hiring <span class="label label-primary">Free</span></a> </li> </ul>  </ul>
        ...
        <div class="panel-body">
          ...
          <div class="row">
            <div class="col-md-12 table-responsive">
			  ...
			  Click here to Go to -> <a href="offer" id="four">OFFERED</a> folder
    </div>
   </div>
  </div>
   </div>
