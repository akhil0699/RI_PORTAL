...
if(isset($_GET['candidateid']) || isset($_POST['allcheck'])){
	...
	$candidateid=$value;
	$recruiter=mysqli_fetch_assoc(mysqli_query($link,"select recruiterid from candidates where id='$candidateid'"));




  if($adminrights){
    $cidsql=mysqli_query($link,"select jobid from submitted_candidates where candidateid='$candidateid'");
    $cjobrow=mysqli_fetch_assoc($cidsql);
    $jmid =  $cjobrow['jobid'];
  

    $jobmemsql=mysqli_query($link,"select memberid from jobs where jobid=$jmid");
	...
	$q2=mysqli_query($link,"select e.email,e.firstname,e.shortlistnotification,d.name from companyprofile d,members e ,submitted_candidates c where  d.id=e.companyid  and e.memberid=c.recruiterid and c.candidateid='$candidateid'");
		while($row2 = mysqli_fetch_array($q2)){
			$array2=$row2;
				}
		$q1=mysqli_query($link,"select a.jobtitle,a.jobid,d.name from jobs a,companyprofile d,members e ,submitted_candidates c where  d.id=e.companyid  and e.memberid=a.memberid and c.jobid=a.jobid and c.candidateid='$candidateid'  ");
          ...
          <div class="row">
            <div class="col-md-12 table-responsive">
              <?php if(!$errormsg){ 
			  			if($content){
						echo $content; 
						echo '<br/><a href="new-msg?rid='.$recruiter['recruiterid'].'&jobid='.$array1['jobid'].'&cid='.$candidateid.'" class=""><b>CLICK HERE TO MESSAGE RECRUITER ON NEXT STEPS IN INTERVIEW PROCESS</b></a>';
						}
					}
			  else echo $errormsg; ?>
    </div>
