...
elseif((!$keywords) && ($sector) && (!$type) && (!$location) &&($status) ){
			...
			$sql = "SELECT * FROM jobs WHERE $block jobsector='$sector' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='Middle East' order by updatedate desc";
	}
elseif((!$keywords) && (!$sector) && ($type) && (!$location) &&($status) ){
 			$param='type='.$type.'&status='.$status;
			$sql = "SELECT * FROM jobs WHERE $block jobtype= '$type' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='Middle East' order by updatedate desc";
	}
elseif((!$keywords) && (!$sector) && (!$type) && ($location) &&($status) ){
 			$param='location='.$location.'&status='.$status;
			$sql = "SELECT * FROM jobs WHERE $block (joblocation like '%$location%' and status='$status') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='Middle East'  order by updatedate desc";
	}	
elseif(($keywords) && ($sector) && (!$type) && (!$location) && (!$status)){
...
elseif((!$keywords) && (!$sector) && ($type) && (!$location) && (!$status)){
 			$param='type='.$type;
			$sql = "SELECT * FROM jobs WHERE $block jobtype= '$type' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='Middle East' order by updatedate desc";
	}	
elseif((!$keywords) && (!$sector) && (!$type) && ($location) && (!$status)){
...
elseif((!$keywords) && (!$sector) && (!$type) && (!$location) && ($status)){
 			$param='status='.$status;
			$sql = "SELECT * FROM jobs WHERE $block status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='Middle East' order by updatedate desc";
	}	

							
else{
$sql="SELECT * FROM jobs WHERE $block status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='Middle East' order by updatedate desc";
...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
     <title>Middle East | Recruiters Hub</title>
...
<h4>Vendor Section (Business)</h4>
  ...
  <li><a href="becomefranchise" role="tab" > <i class="fa fa-database"></i>Become our Franchise <span class="label label-primary">New</span></a> </li>
    ...
    <li><a href="disengaged" role="tab"><i class="fa fa-scissors"></i>DISENGAGED</a>    </li>
...
<div class="account-navi-content">
              ...
              <div class="col-md-12">
                ...
                <div class="row">
                  ...
                  <!--<div class="col-md-2">
                    ...
                    <option value="">Both Permanent and Contract</option>
 ...
 <h3>Total Available Roles in Middle East: <?php echo $total; ?></h3>
...
<!-- <div class="row">
  ...
  <div class="row">
              <div class="col-md-12 table-responsive">
               <div class="panel panel-default">
                <header class="panel-heading wht-bg ui-sortable-handle">
    <h3 class="panel-title"> List of Jobs posted by Employers in Middle East | NO SUBSCRIPTION FEE TO ENGAGE ON JOBS </h3>
  </header>
        <div class="panel-body">
               <?php if(!$content){ ?>
...
$checkpriority=$row['priority'];
		if(($row['fee']=='')&&(($row['country']=='UK')||($row['country']=='Middle East'))&&($row['jobtype']=='Contract')){
