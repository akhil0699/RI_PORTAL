...
//$sql=mysql_query("SELECT * FROM jobs WHERE status not in('Hold','Closed','Filled') and  filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') order by updatedate desc");

$sql="SELECT * FROM jobs WHERE $block status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
...
elseif(($keywords) && ($sector) && ($type) && ($location)  && (!$status) ){
			...
			$sql = "SELECT * FROM jobs WHERE $block (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%')  AND jobsector='$sector' AND joblocation like '%$location%' AND jobtype= '$type' and  status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK'  order by updatedate desc";
	}
...
elseif((!$keywords) && ($sector) && ($type) && (!$location) &&($status) ){
			...
			$sql = "SELECT * FROM jobs WHERE $block jobsector='$sector' AND jobtype= '$type' AND  status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK'  order by updatedate desc";
	}
elseif((!$keywords) && ($sector) && (!$type) && ($location) &&($status) ){
...
elseif(($keywords) && ($sector) && ($type) && (!$location) && (!$status)){
			...
			$sql = "SELECT * FROM jobs WHERE $block (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%')  AND jobsector='$sector' AND jobtype= '$type' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
	}
elseif(($keywords) && (!$sector) && ($type) && ($location) && (!$status)){
...
elseif((!$keywords) && ($sector) && (!$type) && (!$location) &&($status) ){
			...
			$sql = "SELECT * FROM jobs WHERE $block jobsector='$sector' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
	}
elseif((!$keywords) && (!$sector) && ($type) && (!$location) &&($status) ){
 			$param='type='.$type.'&status='.$status;
			$sql = "SELECT * FROM jobs WHERE $block jobtype= '$type' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
	}
elseif((!$keywords) && (!$sector) && (!$type) && ($location) &&($status) ){
 			$param='location='.$location.'&status='.$status;
			$sql = "SELECT * FROM jobs WHERE $block (joblocation like '%$location%' and status='$status') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK'  order by updatedate desc";
	}	
elseif(($keywords) && ($sector) && (!$type) && (!$location) && (!$status)){
 			$param='keywords='.$keywords.'&sector='.$sector;
			$sql = "SELECT * FROM jobs WHERE $block (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%')  AND jobsector='$sector' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
	}
elseif(($keywords) && (!$sector) && ($type) && (!$location) && (!$status)){
 			$param='keywords='.$keywords.'&type='.$type;
			$sql = "SELECT * FROM jobs WHERE $block (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%')  AND  jobtype= '$type' and status not in('Hold','Closed','Filled')  and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
	}	
elseif(($keywords) && (!$sector) && (!$type) && ($location) && (!$status)){
 			$param='keywords='.$keywords.'&location='.$location;
			$sql = "SELECT * FROM jobs WHERE $block (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%')    AND joblocation like '%$location%' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
	}	
elseif((!$keywords) && ($sector) && ($type) && (!$location) && (!$status)){
...
elseif(($keywords) && (!$sector) && (!$type) && (!$location) && (!$status)){
			...
			$sql = "SELECT * FROM jobs WHERE $block (jobtitle like '%$keywords%' || keyskills like '%$keywords%') and status not in('Hold','Closed','Filled')  and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
	}	
elseif((!$keywords) && ($sector) && (!$type) && (!$location) && (!$status)){
 			$param='sector='.$sector;
			$sql = "SELECT * FROM jobs WHERE $block jobsector='$sector' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
	}
elseif((!$keywords) && (!$sector) && ($type) && (!$location) && (!$status)){
 			$param='type='.$type;
			$sql = "SELECT * FROM jobs WHERE $block jobtype= '$type' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
	}	
elseif((!$keywords) && (!$sector) && (!$type) && ($location) && (!$status)){
...
elseif((!$keywords) && (!$sector) && (!$type) && (!$location) && ($status)){
 			$param='status='.$status;
			$sql = "SELECT * FROM jobs WHERE $block status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
	}	

							
else{
$sql="SELECT * FROM jobs WHERE $block status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,request a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and country='UK' order by updatedate desc";
...
<ul class="navbar-right">
  ...
  <img src="images/user.png" class="img-circle" alt="">     <span class="text"><?php echo $name; ?></span><span class="caret"></span> </a>
    <ul class="dropdown-menu" role="menu"><li><a href="company-profile"><i class="fa fa-user"></i>Company Profile</a>   </li>
	  ...
	  /** if($rcountry=='United Kingdom (UK)'){echo'
	   <li><a href="subscription_plans-uk"><i class="fa fa-money"></i> Manage Subscription</a></li>';}
	   else{echo'
...
<div class="account-navi-content">
 ...
 <h3>Total Available Roles in the UK: <?php echo $total; ?></h3>
...
<!-- <div class="row">
  ...
  <div class="row">
              <div class="col-md-12 table-responsive">
               <div class="panel panel-default">
                <header class="panel-heading wht-bg ui-sortable-handle">
    <h3 class="panel-title"> List of Jobs posted by Employers in UK | NO SUBSCRIPTION FEE TO ENGAGE ON JOBS</h3>
  </header>
        <div class="panel-body">
               <?php if(!$content){ ?>
